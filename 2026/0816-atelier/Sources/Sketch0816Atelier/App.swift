...
final class Sketch0816Atelier: Sketch {
    ...
    func draw() {
        ...
        if local < measureFrames {
            ...
            //
            // 影の場面ではシャドウを有効にしたまま測るので、読み戻しが 1 フレーム遅れる
            // （`loadPixels()` は同一フレームを読めず、直前に確定したフレームを返す）。
            // シャドウ深度パスがメインパスの後に走る（`Canvas3D.performShadowPass`）ことも
            // 同じ向きに効く。どちらにせよ標本を差し替えた直後に測ると前の標本を見てしまうので、
