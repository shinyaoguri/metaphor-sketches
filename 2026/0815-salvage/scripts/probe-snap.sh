#!/bin/bash
# 実行中の 0815-salvage へ Probe リクエストを投げ、1 フレーム分の
# 画像 (PNG) とメタデータ (frame.json) を取り出す。
#
#   scripts/probe-snap.sh <label> [出力先ディレクトリ]
#
# 前提: 別プロセスで METAPHOR_PROBE=1 の 0815-salvage が動いていること
#       (scripts/run.sh か soak.sh が起動する)。
#
# Probe の performance 節は単一フレーム経路にしか載らないため、
# 連続キャプチャ (sequence) ではなく request 方式で取る (CONTRACT.md 契約点 4)。
set -euo pipefail
...
print("scene={} stage={} transitions={} cycles={} uptime={}".format(
    c.get("scene"), c.get("stage"), c.get("sceneTransitions"),
    c.get("cycles"), c.get("uptimeSec")))
print("assets live={} loads={} frees={} lastLoadMs={} cache={}".format(
    c.get("assetsLive"), c.get("assetsLoads"), c.get("assetsFrees"),
    c.get("assetLoadMs"), c.get("assetCacheMode")))
if c.get("assetFailures"):
    print("asset failures:", c["assetFailures"])
print("play collected={} lives={}".format(c.get("collectedTotal"), c.get("lives")))
if d.get("warnings"):
